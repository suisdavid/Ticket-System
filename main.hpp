...
{
    int hour,minute;
    date(int h=0,int m=0){hour=h;minute=m;}
...
void outputdate(pair<int,date>d)
{
    ...
    cout<<d.first<<" ";
    if (d.second.hour<10){cout<<0;}
    cout<<d.second.hour<<":";
    if (d.second.minute<10){cout<<0;}
...
struct train//854
{
    ...
    {
        charmake(trainid, _trainid);uid=_hash(_trainid);
...
struct train_station_map//32
{
    ...
    {
        min_arrival=_min_arrival;min_departure=_min_departure;
        id=_id;len=_len;
    }
...
{
    char trainid[21];
    long long uid;int price,len,l,r;
    ...
    train_da(string _trainid="",int _price=0,int _len=0,pair<int,date>_departure=pair<int,date>(),pair<int,date>_arrival=pair<int,date>(),int i=0,int j=0)
    {
        ...
        price=_price;len=_len;departure=_departure;arrival=_arrival;l=i;r=j;
        uid=_hash(_trainid);
    }
...
{
    long long uid;int timestamp,order,l,r,num;
    pending_request(long long _uid=0,int _timestamp=0,int _order=0,int _l=0,int _r=0,int _num=0)
    {
        uid=_uid;timestamp=_timestamp;order=_order;l=_l;r=_r;num=_num;
    }
