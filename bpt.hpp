...
namespace database
{
    ...
    private:
        ...
        {
            int id = 0;
            while (id < p.cnt && p.val[id] < v)
            {
                id++;
            }
            if (id < p.cnt && p.val[id] == v)
            {
                return;
            }
        ...
        pair<node, node> node_split(node &p)
        {
            ...
            for (int i = 0; i < ((M + 1) >> 1); i++)
            {
                a.val[i] = p.val[i];
                a.child[i] = p.child[i];
                b.val[i] = p.val[i + ((M + 1) >> 1)];
                b.child[i] = p.child[i + ((M + 1) >> 1)];
            }
            a.cnt = b.cnt = ((M + 1) >> 1);
        ...
        pair<int, pair<long long, T>> ins(int u, pair<long long, T> v, int rt = 0)
        {
            ...
            if (p.is_data)
            {
                ...
                else
                {
                    ...
                    node nd = node();
                    int id = new_node(nd);
                    new_nodes.first.is_data = id;
                    new_nodes.second.is_data = p.is_data;
                ...
                if (res.first)
                {
                    ...
                    else
                    {
                        ...
                        if (u == rt)
                        {
                            ...
                            np.child[0] = rt;
                            np.child[1] = id;
                            np.val[0] = new_nodes.first.val[0];
                            np.val[1] = new_nodes.second.val[0];
                            int newrt = new_node(np);
                            f.WriteInfo(newrt, 0);
                            return make_pair(0, make_pair(0, T()));
                        }
                        return make_pair(id, new_nodes.second.val[0]);
        ...
        void node_merge(node &p, node &q)
        {
            ...
            {
                p.val[i] = q.val[i - p.cnt];
                p.child[i] = q.child[i - p.cnt];
            }
            p.cnt += q.cnt;
        }
        int try_del(int u, pair<long long, T> v, int fa, int id, node &p, node &father)
        {
            ...
            else
            {
                ...
                {
                    node nex;
                    f.read(nex, father.child[id + 1]);
                    if (nex.cnt > (M + 1) / 2)
                    {
                        node_add(p, nex.val[0], nex.child[0]);
                        f.update(p, u);
                        node_del(nex, make_pair(-2, T()));
                        f.update(nex, father.child[id + 1]);
                        father.val[id + 1] = nex.val[0];
                        father.val[id] = p.val[0];
                        return -1;
                    }
                    else if (!id)
                ...
                {
                    node pre;
                    f.read(pre, father.child[id - 1]);
                    if (pre.cnt > (M + 1) / 2)
                    {
                        node_add(p, pre.val[pre.cnt - 1], pre.child[pre.cnt - 1]);
                        f.update(p, u);
                        node_del(pre, make_pair(-2 - pre.cnt + 1, T()));
    ...
    public:
        BPT(string filename = "a.out")
        {
            if (not_exist(filename))
            {
                ...
                node root = node(), p = node(), q = node();
                root.cnt = 2;
                root.val[0] = make_pair(-(M + 1) / 2, T());
                root.val[1] = make_pair(mod, T());
                for (int i = 0; i < (M + 1) / 2; i++)
                {
                    p.val[i] = make_pair(i - (M + 1) / 2, T());
                    q.val[i] = make_pair(mod + i, T());
                }
                p.cnt = q.cnt = (M + 1) / 2;
                p.is_data = p.child[0] = 2;
                q.is_data = q.child[0] = 1;
                root.child[0] = new_node(p);
                root.child[1] = new_node(q);
