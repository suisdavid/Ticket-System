...
namespace user_op
{
    ...
    void login()
    {
        ...
        if (_user.first == 0 || string(_user.second.password) != password || user_database::login(username))
        {
            response(-1);
            return;
        }
    ...
    void logout()
    {
        ...
        cin >> param >> username;
        pair<bool, user> _user = user_database::query(username);
        if (_user.first == 0 || user_database::login(username) == 0)
        {
            response(-1);
            return;
        }
    ...
    void query_profile()
    {
        ...
        pair<bool, user> current_user = user_database::query(cur);
        if (current_user.first == 0)
        {
            response(-1);
            return;
        }
        pair<bool, user> queried_user = user_database::query(username);
        if (queried_user.first == 0 || !(current_user.second.privilege > queried_user.second.privilege || current_user.second.uid == queried_user.second.uid))
        {
            response(-1);
            return;
        }
        response();
        queried_user.second.print();
    }
    ...
    void modify_profile()
    {
        ...
        if (privilege >= current_user.second.privilege)
        {
            response(-1);
            return;
        }
        if (password != "")
        {
            charmake(queried_user.second.password, password);
        }
        if (name != "")
        {
            charmake(queried_user.second.name, name);
        }
        if (mail != "")
        {
            charmake(queried_user.second.mail, mail);
        }
        if (privilege != -1)
        {
            queried_user.second.privilege = privilege;
        }
...
namespace train_op
{
    void add_train()
    {
        ...
        vector<int> travelTimes, stopoverTimes, prices;
        char type;
        date startTime;
        ...
        for (int i = 1; i <= 10; i++)
        {
            ...
            {
                read_separate_int(travelTimes);
            }
            else if (param == "-o")
            {
                read_separate_int(stopoverTimes);
            }
            else if (param == "-d")
            {
        ...
        if (train_database::query(train_id).first)
        {
            response(-1);
            return;
        }
    ...
    void query_train()
    {
        ...
        train &_train = res.second;
        int addition = day - _train.start;
        response();
        cout << train_id << " " << _train.type << endl;
    ...
    void query_ticket()
    {
        ...
        vector<train_da> trains = train_database::find_all(sid, tid);
        int sz = trains.size();
        vector<train_da> new_trains;
        for (int i = 0; i < sz; i++)
        {
            if (trains[i].departure.first > day || trains[i].departure.first + trains[i].len < day)
            {
                continue;
            }
    ...
    struct transfer_ans
    {
        ...
        void checkmin(transfer_ans &rhs, int seed)
        {
            ...
            if (trainid1 < rhs.trainid1)
            {
                return;
            }
            else if (trainid1 > rhs.trainid1)
            {
                *this = rhs;
                return;
            }
            if (trainid2 < rhs.trainid2)
            {
                return;
            }
            else if (trainid2 > rhs.trainid2)
            {
                *this = rhs;
                return;
            }
    ...
    void query_transfer()
    {
        ...
        long long sid = _hash(s), tid = _hash(t);
        vector<long long> trains1 = train_database::get_all_trains(sid);
        int sz = trains1.size();
        transfer_ans ans = transfer_ans();
        bool found = 0;
        for (int i = 0; i < sz; i++)
        {
            long long trid1 = trains1[i];
            train _train = train_database::get_train(trid1);
            int n = _train.station_num, len = _train.len, price = 0, j = 0;
            while (_train.station_uid[j] != sid)
            {
                j++;
            }
            if (j == n - 1)
            ...
            if (m1.min_departure.first > day || m1.min_departure.first + len < day)
            {
                continue;
            }
...
namespace request_op
{
    void buy_ticket()
    {
        string param, username, trainid, s, t, flag = "false";
        int num, day;
        while (CanCin())
        {
            cin >> param;
        ...
        if (user_database::login(username) == 0 || train_database::released(trainid) == 0)
        {
            response(-1);
            return;
        }
        ...
        if (addition < 0 || addition > _train_station_map.len)
        {
            response(-1);
            return;
        }
        if (num > train_database::query_seat(trid, maxday, 0, 1))
        {
            response(-1);
            return;
        } // 大于总票数，直接寄
        pair<int, date> arrival = train_database::query_train_station_map(trid, tid).min_arrival;
        departure.first += addition;
        arrival.first += addition;
    ...
    void query_order()
    {
        string param, username;
        cin >> param >> username;
        if (user_database::login(username) == 0)
        {
            response(-1);
            return;
        }
    ...
    void refund_ticket()
    {
        ...
        if (user_database::login(username) == 0 || n <= 0)
        {
            response(-1);
            return;
        }
        ...
        int l = _request.l, r = _request.r, num = _request.num, day = _request.day;
        long long trid = _hash(_request.trainid);
